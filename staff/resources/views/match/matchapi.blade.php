...
<?php
$status = "Finished at ".date("d-m-Y H:i", strtotime($match->started_at) + $match->duration);
?>
            ...
            <div class="row">
                ...
                <div class="ibox">
                    <div class="ibox-content">
                        <div class="col-md-6">
                            <h2><?php echo $match->radiant; ?> VS. <?php echo $match->dire; ?> <?php if (isset($duration)) { ?>At minute <?php echo round($duration/60); ?><?php } ?></h2>
                        </div>
                        ...
                        <div class="col-md-6 text-right">
                            League ID: {{ isset($match->leagueid) ? $match->leagueid : ''}}
                        </div>
                    ...
                    <div class="row">
                        <div class="col-md-4"><?php echo $match->radiant; ?> VS. <?php echo $match->dire; ?></div>
                    ...
                    <div class="ibox">
                       <div class="ibox-content table-responsive">
                        <table class="table">
                            ...
                            <tbody>
                                <?php if (count($match->radiant_players) > 0) { ?>
                                <?php foreach ($match->radiant_players as $player) { 
                                    if ($player->hero_id > 0) {
                                    ...
                                    ?>
                                    <tr>
                                        <td><a href="{{ groute('players.view', 'current', [$player->account_id]) }}">{{ $player->name }}</a></td>
                                        <td>{{ $hero }}</td>
                                        <td>{{ $player->level }}</td>
                                        <td>{{ $player->kills }}</td>
                                        <td>{{ $player->deaths }}</td>
                                        ...
                                        <td>{{ isset($itemMap[$player->item0]) ? $itemMap[$player->item0] : '' }}</td>
                                        <td>{{ isset($itemMap[$player->item1]) ? $itemMap[$player->item1] : '' }}</td>
                                        <td>{{ isset($itemMap[$player->item2]) ? $itemMap[$player->item2] : '' }}</td>
                                        <td>{{ isset($itemMap[$player->item3]) ? $itemMap[$player->item3] : '' }}</td>
                                        <td>{{ isset($itemMap[$player->item4]) ? $itemMap[$player->item4] : '' }}</td>
                                        <td>{{ isset($itemMap[$player->item5]) ? $itemMap[$player->item5] : '' }}</td>
                                    </tr>
                                <?php 
                                    } 
                                }
                    ...
                    <div class="ibox">
                       <div class="ibox-content table-responsive">
                        <table class="table">
                            ...
                            <tbody>
                                <?php if (count($match->dire_players) > 0) { ?>
                                <?php foreach ($match->dire_players as $player) { 
                                    if ($player->hero_id > 0) {
                                        if (isset($heroMap[$player->hero_id]))
                                            $hero = $heroMap[$player->hero_id];
                                        else
