...
@Component({
  selector: 'app-player-lol-spotlight',
  templateUrl: './player-lol-spotlight.component.html',
  styles: [`
    .heroes-cont img {
       max-width:47px;
       max-height:47px;
...
export class PlayerLolSpotlightComponent implements OnInit {
  @Input() playerId;
  @Input() game;
  options: Object;
  categories = [];
  ...
  ngOnInit() {
    this._apiService.getPlayer(this.game, this.playerId).subscribe((res)=>{
      ...
      //  Chart position on the lol page
      let chartPosition = (this.game == 'lol') ? -20 : -34;
